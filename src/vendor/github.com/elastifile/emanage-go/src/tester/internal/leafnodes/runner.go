...
func mustFuncType(body interface{}, codeLocation types.CodeLocation) reflect.Type {
...
func mustFuncArgChan(funcType reflect.Type, codeLocation types.CodeLocation) {
	if !(funcType.In(0).Kind() == reflect.Chan && funcType.In(0).Elem().Kind() == reflect.Interface) {
...
func newRunner(body interface{}, codeLocation types.CodeLocation, timeout time.Duration, failer *failer.Failer, nodeType types.SpecComponentType, componentIndex int) *runner {
	...
	case 1:
		mustFuncArgChan(bodyType, codeLocation)
