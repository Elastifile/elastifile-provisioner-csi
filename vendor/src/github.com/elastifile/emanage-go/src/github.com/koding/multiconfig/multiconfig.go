...
type FieldSetter interface {
	SetValue(v string, ctx interface{}) error
}
...
func FieldSet(field *structs.Field, v string, ctx interface{}) error {
	// Implement a specific interface to allow a user to provide his own setter.

	if field.CanAddr() {
		if setter, ok := field.Addr().Interface().(FieldSetter); ok {
			return setter.SetValue(v, ctx)
		}
	}

	if setter, ok := field.Value().(FieldSetter); ok {
		return setter.SetValue(v, ctx)
	}
	...
	default:
		return errors.Errorf("multiconfig: field '%s' has unsupported type: %s", field.Name(), field.Kind())
	}
